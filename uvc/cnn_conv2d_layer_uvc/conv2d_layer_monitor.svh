//--------------------------------------------------------------------------------
// conv2d_layer_monitor class
//
// This UVM monitor passively observes the CNN convolution-layer DUT interface
// through the virtual interface handle stored in conv2d_layer_config.
//
...
// a conv2d_layer_output_seq_item transaction, and publishes that transaction to other
// verification components (scoreboard / coverage) via an analysis port.
//--------------------------------------------------------------------------------
class conv2d_layer_monitor extends uvm_monitor;
    `uvm_component_utils(conv2d_layer_monitor)

    // Convolution Layer UVC configuration
    conv2d_layer_config m_config;

    // Analysis port to scoreboard / coverage
    uvm_analysis_port #(conv2d_layer_output_seq_item) m_analysis_port;

    // Image ID tracker for multiple images
    int unsigned img_id;
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db #(conv2d_layer_config)::get(this,"","conv2d_layer_config", m_config))
            `uvm_fatal(get_name(), "Cannot find conv2d_layer_config")
    ...
    //   3) If valid:
    //       a) Create a new monitor transaction (conv2d_layer_output_seq_item).
    ...
    task run_phase(uvm_phase phase);
        conv2d_layer_output_seq_item img_transac;    
      ...
      if (m_config.m_vif.output_data_valid) begin
        img_transac = conv2d_layer_output_seq_item::type_id::create("img_transac", this);
